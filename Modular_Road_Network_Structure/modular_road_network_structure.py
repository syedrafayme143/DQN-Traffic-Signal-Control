...
def create_node_xmlfile(num_nodes, len_edges):
    ...
    node_file_path = f'intersection/Ingolstadt_{num_nodes}_Nodes.nod.xml'
...
def create_edge_xmlfile(square_grid, square_grid_list, square_grid_list_original):
    xml = minidom.Document()
    ...
    edge_file_path = f'intersection/Ingolstadt_{int(len(edge_combinations) / 2)}_Edges.edg.xml'
...
def create_connection_xmlfile(square_grid, square_grid_edges):
    xml = minidom.Document()
    ...
    connection_file_path = f'intersection/Ingolstadt_{len(square_grid_edges)}_Connections.con.xml'
...
def create_tllogic_xmlfile(square_grid):
    xml = minidom.Document()
    ...
    tllogic_file_path = f'intersection/Ingolstadt_{len(square_grid_nodes_list)}_TrafficLights.tll.xml'
...
def create_env_xmlfile(node_files, edge_files, connection_files, tllogic_files):
    env_file = f'intersection/Ingolstadt_Environment.net.xml'
...
def create_modular_road_network(number_nodes, length_edges=100):

    files = glob.glob('intersection/*')
    for f in files:
        os.remove(f)

    square_grid, square_grid_list, square_grid_list_original, node_file = create_node_xmlfile(num_nodes=number_nodes, len_edges=length_edges)
    square_grid_edges, edge_file = create_edge_xmlfile(square_grid=square_grid, square_grid_list=square_grid_list, square_grid_list_original=square_grid_list_original)
    connection_file = create_connection_xmlfile(square_grid=square_grid, square_grid_edges=square_grid_edges)
    tllogic_file = create_tllogic_xmlfile(square_grid=square_grid)
    create_env_xmlfile(node_files=node_file, edge_files=edge_file, connection_files=connection_file, tllogic_files=tllogic_file)


if __name__ == '__main__':
    create_modular_road_network(50,200)